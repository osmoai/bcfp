...
/**
 * Atom and Bond invariants (matching Python implementation)
 */
class Invariants {
public:
    // Compute atom invariant hash
    static uint32_t atom_invariant(const RDKit::Atom* atom, bool useChirality);
    
    // Compute bond invariant hash
 ...
 * Uses RDKit's boost::hash_combine style (same as MorganGenerator)
